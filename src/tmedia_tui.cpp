...
void TMediaCursesRenderer::render(TMediaProgramState& tmps, const TMediaProgramSnapshot& sshhot) {
  ...
  if (COLS < MIN_RENDER_COLS || LINES < MIN_RENDER_LINES) {
    erase();
      ...
      this->render_tui_fullscreen(tmps, sshhot);
  } else if (COLS < 60) {
    this->render_tui_compact(tmps, sshhot);
  } else {
    this->render_tui_large(tmps, sshhot);
  }

  this->last_frame_dims = Dim2(sshhot.frame.get_width(), sshhot.frame.get_height());
}

void TMediaCursesRenderer::render_tui_fullscreen(TMediaProgramState& tmps, const TMediaProgramSnapshot& sshhot) {
  render_pixel_data(sshhot.frame, 0, 0, COLS, LINES, tmps.vom, tmps.scaling_algorithm, tmps.ascii_display_chars);
  tmps.req_frame_dim = Dim2(COLS, LINES);
}

void TMediaCursesRenderer::render_tui_compact(TMediaProgramState& tmps, const TMediaProgramSnapshot& sshhot) {
  static constexpr int CURRENT_FILE_NAME_MARGIN = 5;
  render_pixel_data(sshhot.frame, 2, 0, COLS, LINES - 4, tmps.vom, tmps.scaling_algorithm, tmps.ascii_display_chars);
  ...
  const std::string current_plist_media_str = get_media_file_display_name(tmps.plist.current(), this->metadata_cache);
  ...
  if (sshhot.media_type == MediaType::VIDEO || sshhot.media_type == MediaType::AUDIO) {
    wfill_box(stdscr, LINES - 3, 0, COLS, 1, '~');
    wprint_playback_bar(stdscr, LINES - 2, 0, COLS, sshhot.media_time_secs, sshhot.media_duration_secs);

    std::vector<std::string_view> bottom_labels;
    const std::string_view playing_str = sshhot.playing ? ">" : "||";
    ...
    if (sshhot.has_audio_output) bottom_labels.push_back(volume_str);
...
void TMediaCursesRenderer::render_tui_large(TMediaProgramState& tmps, const TMediaProgramSnapshot& sshhot) {
  static constexpr int CURRENT_FILE_NAME_MARGIN = 5;
  render_pixel_data(sshhot.frame, 2, 0, COLS, LINES - 4, tmps.vom, tmps.scaling_algorithm, tmps.ascii_display_chars);
  tmps.req_frame_dim = Dim2(COLS, LINES - 4);
  ...
  if (tmps.plist.size() == 1) {
    ...
    if (tmps.plist.can_move(PlaylistMvCmd::REWIND)) {
      ...
      const std::string rewind_media_file_display_string = get_media_file_display_name(tmps.plist.peek_move(PlaylistMvCmd::REWIND), this->metadata_cache);
    ...
    if (tmps.plist.can_move(PlaylistMvCmd::SKIP)) {
      ...
      const std::string skip_display_string = get_media_file_display_name(tmps.plist.peek_move(PlaylistMvCmd::SKIP), this->metadata_cache);
  ...
  if (sshhot.media_type == MediaType::VIDEO || sshhot.media_type == MediaType::AUDIO) {
    ...
    std::vector<std::string_view> bottom_labels;
    const std::string_view playing_str = sshhot.playing ? "PLAYING" : "PAUSED";
