...
int AudioRingBuffer::get_nb_can_read() {
  if (this->m_head < this->m_tail) return this->m_tail - this->m_head;
  if (this->m_tail < this->m_head) return this->m_size - this->m_head + this->m_tail;
  return 0;
}

int AudioRingBuffer::get_nb_can_write() {
  if (this->m_head < this->m_tail) return this->m_size - this->m_tail + this->m_head;
  if (this->m_tail < this->m_head) return this->m_head - this->m_tail;
  return 0;
}

void AudioRingBuffer::read_into(int nb_frames, float* out) {
  if (this->get_nb_can_read() < nb_frames) throw std::runtime_error("duhduh");
...
void AudioRingBuffer::peek_into(int nb_frames, float* out) {
  if (this->get_nb_can_read() < nb_frames) throw std::runtime_error("duhduhduh stupid");
...
void AudioRingBuffer::write_into(int nb_frames, float* in) {
  if (this->get_nb_can_write() < nb_frames) throw std::runtime_error("do de lit do");
  for (int i = 0; i < nb_frames * this->m_nb_channels; i++) {
