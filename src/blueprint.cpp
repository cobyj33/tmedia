...
/*         const static int nb_displayModes = 3; */
...
/*             /1* this->videoFrames.reserve(FRAME_RESERVE_SIZE); *1/ */
/*             /1* this->audioFrames.reserve(FRAME_RESERVE_SIZE); *1/ */
...
/*                 /1* std::this_thread::sleep_for(waitDuration); *1/ */
...
/*             /1* printw("Target Target PTS: %f", state->playback->time / state->audioTimeBase); *1/ */
/*             /1* refresh(); *1/ */
...
/*                     float** originalValues = copy_samples(currentAudioFrame->data, currentAudioFrame->ch_layout, currentAudioFrame->nb_samples); */
/*                     float** resampledValues = alloc_samples(currentAudioFrame->linesize, currentAudioFrame->ch_layout, currentAudioFrame->nb_samples); */

                    
/*                     /1* if (debug) { *1/ */
/*                     /1*     printw("Current nb_samples: %d, Stretch Amount: %f, bytesPerSample: %d, inputtedBytes: %d, bytesToWrite: %d\n", currentAudioFrame->nb_samples, stretchAmount, bytesPerSample, currentAudioFrame->nb_samples * bytesPerSample, samplesToWrite * bytesPerSample); *1/ */
/*                     /1* } *1/ */

/*                     /1* int channelCount = currentAudioFrame->ch_layout.nb_channels; *1/ */
/*                     /1* float orignalSampleBlocks[currentAudioFrame->nb_samples][channelCount]; *1/ */
/*                     /1* float finalSampleBlocks[samplesToWrite][channelCount]; *1/ */

/*                     /1* for (int i = 0; i < currentAudioFrame->nb_samples * channelCount; i++) { *1/ */
/*                     /1*     orignalSampleBlocks[i / channelCount][i % channelCount] = originalValues[0][i]; *1/ */
/*                     /1* } *1/ */
                    
/*                     /1* if (shouldStretchAudioSegment) { *1/ */
/*                     /1*     float currentSampleBlock[channelCount]; *1/ */
/*                     /1*     float lastSampleBlock[channelCount]; *1/ */
/*                     /1*     float currentFinalSampleBlockIndex = 0; *1/ */

/*                     /1*     if (debug) { *1/ */
/*                     /1*         printw("Stretching audio segment by factor of %f\n", stretchAmount); *1/ */
/*                     /1*     } *1/ */

/*                     /1*     for (int i = 0; i < currentAudioFrame->nb_samples; i++) { *1/ */
/*                     /1*         for (int cpy = 0; cpy < channelCount; cpy++) { *1/ */
/*                     /1*             currentSampleBlock[cpy] = orignalSampleBlocks[i][cpy]; *1/ */
/*                     /1*         } *1/ */

/*                     /1*         if (i != 0) { *1/ */
/*                     /1*             for (int currentChannel = 0; currentChannel < channelCount; currentChannel++) { *1/ */
/*                     /1*                 float sampleStep = (currentSampleBlock[currentChannel] - lastSampleBlock[currentChannel]) / stretchAmount; *1/ */ 
/*                     /1*                 float currentSampleValue = lastSampleBlock[currentChannel]; *1/ */

/*                     /1*                 for (int finalSampleBlockIndex = (int)(currentFinalSampleBlockIndex); finalSampleBlockIndex < (int)(currentFinalSampleBlockIndex + stretchAmount); finalSampleBlockIndex++) { *1/ */
/*                     /1*                     finalSampleBlocks[finalSampleBlockIndex][currentChannel] = currentSampleValue; *1/ */
/*                     /1*                     currentSampleValue += sampleStep; *1/ */
/*                     /1*                 } *1/ */

/*                     /1*             } *1/ */
/*                     /1*         } *1/ */

/*                     /1*         currentFinalSampleBlockIndex += stretchAmount; *1/ */
/*                     /1*         for (int cpy = 0; cpy < channelCount; cpy++) { *1/ */
/*                     /1*             lastSampleBlock[cpy] = currentSampleBlock[cpy]; *1/ */
/*                     /1*         } *1/ */
/*                     /1*     } *1/ */

/*                     /1* } else if (shouldShrinkAudioSegment) { *1/ */
/*                     /1*     float currentOriginalSampleBlockIndex = 0; *1/ */
/*                     /1*     float stretchStep = state->playback->speed; *1/ */

/*                     /1*     if (debug) { *1/ */
/*                     /1*         printw("Shrinking audio segment by factor of %f\n", stretchAmount); *1/ */
/*                     /1*     } *1/ */

/*                     /1*     for (int i = 0; i < samplesToWrite; i++) { *1/ */
/*                     /1*         float average = 0.0; *1/ */
/*                     /1*         for (int ichannel = 0; ichannel < channelCount; ichannel++) { *1/ */
/*                     /1*            average = 0.0; *1/ */
/*                     /1*            for (int origi = (int)(currentOriginalSampleBlockIndex); origi < (int)(currentOriginalSampleBlockIndex + stretchStep); origi++) { *1/ */
/*                     /1*                average += orignalSampleBlocks[origi][ichannel]; *1/ */
/*                     /1*            } *1/ */
/*                     /1*            average /= stretchStep; *1/ */
/*                     /1*            finalSampleBlocks[i][ichannel] = average; *1/ */
/*                     /1*         } *1/ */

/*                     /1*         currentOriginalSampleBlockIndex += stretchStep; *1/ */
/*                     /1*     } *1/ */

/*                     /1* } *1/ */

                    
/*                     /1* for (int i = 0; i < samplesToWrite * channelCount; i++) { *1/ */
/*                     /1*     finalValues[0][i] = finalSampleBlocks[i / channelCount][i % channelCount]; *1/ */
/*                     /1* } *1/ */
...
/* const VideoStateDisplayMode VideoState::displayModes[3] = { MOVIE, VIDEO_DEBUG, AUDIO_DEBUG }; */
